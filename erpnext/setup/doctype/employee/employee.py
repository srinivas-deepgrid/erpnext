...
class Employee(NestedSet):
	...
	shift_cycle_GS = ['GS']
	shift_cycle_1 = ['A', 'C', 'B']  # Shift pattern for the three weeks
	shift_cycle_2 = ['B', 'A', 'C']  # Shift pattern for the three weeks
	shift_cycle_3 = ['C', 'B', 'A']  # Shift pattern for the three weeks
	...
	def validate(self):
		...
		self.set_shift_cycle()
	
	@frappe.whitelist()
	def set_shift_cycle(self):
		shift = self.default_shift
		# print("selected shift-----",shift)
		if shift == 'GS':
			self.shift_cycle = "Shift Cycle GS"
			return self.shift_cycle

		# get current time stamp while assigning shift 
		current_date = datetime.now().date()
		# current_date = datetime(2023, 5, 30).date()
		shift_cycle_index = self.get_shift_cycle_for_date(current_date)

		# Determine the shift cycle based on the provided shift
		if shift == 'A':
			shift_cycles_index = shift_cycle_index % 3  # 3 shift cycles: A, C, B
		elif shift == 'B':
			shift_cycles_index = (shift_cycle_index + 1) % 3
		elif shift == 'C':
			shift_cycles_index = (shift_cycle_index + 2) % 3
		else:
			frappe.throw(_("Invalid shift selected."))

		shift_cycles = ["Shift Cycle 1","Shift Cycle 2","Shift Cycle 3"]
		
		# print("shift_cycles_index---",shift_cycles_index)
		if -1<shift_cycles_index<3:
			self.shift_cycle = shift_cycles[shift_cycles_index]

		# print("self.shift_cycle---",self.shift_cycle)

		return self.shift_cycle

	def get_shift_cycle_for_date(self,input_date):
		start_date = datetime(2023, 5, 29).date()  # Starting date for the shift pattern
		# Calculate the number of days between the start date and input date
		days_diff = (input_date - start_date).days
		# Calculate the index of the shift cycle based on the number of weeks passed
		shift_cycle_index = days_diff // 7 
		return shift_cycle_index
...
@frappe.whitelist()
def get_shift_based_on_shift_cycle(shift_cycle,current_date=None):
	# print("today called----","shift_cycle---",shift_cycle)
	start_date = datetime(2023, 5, 29).date()  # Starting date for the shift pattern A,C,B
	shift_patterns = {
		"Shift Cycle 1" : ['A', 'C', 'B'],
		"Shift Cycle 2" : ['B', 'A', 'C'],
		"Shift Cycle 3" : ['C', 'B', 'A'],
	...
	if shift_cycle:
		if shift_cycle == "Shift Cycle GS":
			return "GS"
		else:
			shift_pattern = shift_patterns[shift_cycle]
			# print("shift_pattern---",shift_pattern)
	else:
		return None
	...
	# print("curent date",current_date)

	# Calculate the number of days between the start date and current date
	days_diff = (current_date.date() - start_date).days

	# Calculate the index of the shift cycle based on the number of weeks passed
	shift_index = days_diff // 7 % len(shift_pattern)

	# Retrieve the shift based on the shift index
	shift = shift_pattern[shift_index]
